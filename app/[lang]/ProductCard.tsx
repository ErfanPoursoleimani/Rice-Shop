...
const ProductCard =  ({ product }: Props) => {
  ...
  const { dict, cartProducts } = useData()
  ...
  return (
    <div className={`w-60 h-80 text-[var(--light-text)] space-y-3 pt-15 pb-7 px-5 relative flex justify-center items-center gap-2 rounded-[30px] overflow-clip`} onMouseEnter={() => handleHoverOverCard('enter')} onMouseLeave={() => handleHoverOverCard("leave")}>
      <div className='absolute w-full h-[500px] bg-[var(--foreground)] z-2'></div>
      {/* <Image className={`rounded-[4px] absolute w-70 h-95 z-1 ${scale} duration-500`} src={}} width={1000} height={1000} alt="product"></Image> */}
      <div className='z-3 w-full h-full flex flex-col justify-between items-center'>
        ...
        <div className={classnames({'text-left': !(lang === 'fa' || lang === 'ar'), 'text-right': lang === 'fa' || lang === 'ar'  , 'w-full ': true})}>
          <p>{price} <span className='text-[var(--sub-light-text)] text-[13px]'>({perkg})</span> {productPrice} {currency}</p>
