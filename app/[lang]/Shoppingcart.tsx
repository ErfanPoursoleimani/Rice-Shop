...
const Shoppingcart = ({setShoppingcartDisplay, setSignInDisplay} : Props) => {
  ...
  return (
    <div>
      <div className={classNames({'left-to-right-animation': !(lang === 'fa' || lang === 'ar'), "right-to-left-animation": (lang === 'fa' || lang === 'ar') ,'overflow-scroll fixed top-[80px] p-12 z-101 w-[100vw] h-[calc(100vh-80px)] backdrop-blur-3xl bg-[var(--light-foreground)] flex flex-col items-end': true})}>
        ...
        <button style={cartProducts.length === 0 ? {display: 'none'} : {}} className="mt-20 p-2 bg-[#105200da] rounded-[5px] cursor-pointer text-[var(--light-text)]" onClick={userId === null ? handleSignInDisplay : handleOrderPageDisplay}>
          {userId === null ? <SignInButton /> : "ادامه"}
        </button>
      </div>
