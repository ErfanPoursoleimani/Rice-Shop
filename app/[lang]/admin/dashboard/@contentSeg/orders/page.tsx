...
const page = () => {
  ...
  const { dict, isRTL, orders } = useDataStore()
  ...
  const baseNavLinks = [
    {id: 1, label: "inProgress", href: `/${lang}/admin/dashboard/orders?activeTab=in_progress`, searchParamsValue: "in_progress", },
    {id: 2, label: "sent", href: `/${lang}/admin/dashboard/orders?activeTab=sent`, searchParamsValue: "sent", },
    {id: 3, label: "canceled", href: `/${lang}/admin/dashboard/orders?activeTab=cancelled`, searchParamsValue: "cancelled", },
  ]
  ...
  return (
    <div className={`bottom-to-top-animation min-h-100 max-md:fixed max-md:min-h-full max-md:w-full max-md:bg-white max-md:p-5 flex flex-col flex-1 justify-stretch md:border-1 border-neutral-300 rounded-[10px]`}>
      <div className={`md:mx-5 md:my-5 flex justify-between items-center h-10 ${isRTL ? "flex-row-reverse" : "flex-row"}`}>
            <p className='font-medium md:border-b-3 border-yellow-400 md:pb-4'>Orders</p>
            {isRTL 
              ? <MdArrowBackIos className='md:hidden' onClick={handleBackAction} /> 
              : <MdArrowForwardIos className='md:hidden' onClick={handleBackAction} /> }
      </div>
      ...
      <div className={`text-[13px] flex-1 flex justify-center gap-5 items-stretch ${isRTL ? "text-end" : "text-start"}`}>
        ...
        : orders && !(orders.filter((order: Order) => order.status === orderStatusFilter))
        ...
        : orders.filter((order: Order) => order.status === orderStatusFilter).map((order: Order) => (
