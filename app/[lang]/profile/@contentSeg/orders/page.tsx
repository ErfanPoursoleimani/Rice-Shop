  ...
  return (
    <div className={`bottom-to-top-animation min-h-60 max-md:fixed max-md:min-h-full max-md:w-full max-md:bg-white max-md:p-5 flex flex-col flex-1 justify-stretch md:border-1 border-neutral-300 rounded-[10px]`}>
      ...
      <div className={`flex text-[13px] justify-stretch border-b-1 border-neutral-200 ${isRTL ? "flex-row-reverse" : "flex-row"}`}>
        {navLinks.map((link) => (
          <Link key={link.id} className={`flex-1 flex justify-center items-center min-h-10 ${ searchParams.get('activeTab') === link.searchParamsValue ? "text-[var(--theme)] border-b-5 border-[var(--theme)]" : null }`} href={link.href}>{link.label}</Link>
        ))}
      </div> 
      <div className='flex-1 p-4 space-y-4'>
        ...
        : !(user!.orders.filter((order: Order) => order.status === orderStatusFilter))
        ?  <div className='text-[15px] min-h-60 flex-1 flex justify-center items-center '>
            <div className='flex flex-col justify-center items-center'>
              <Image width={100} height={100} src={'@/public/images/order-empty.svg'} alt={''} />
