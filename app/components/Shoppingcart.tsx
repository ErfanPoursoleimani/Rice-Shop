...
const Shoppingcart = ({addedToCartProducts, setShoppingcartDisplay, setSignInDisplay, phoneNumber} : {addedToCartProducts: {id: number, label: string, price: string, count: number}[], setShoppingcartDisplay: Function, setSignInDisplay: Function, phoneNumber: string}) => {
  ...
  return (
    <div>
      <div className='hamburger-nav-animation overflow-scroll fixed top-[80px] p-12 z-101 w-[100vw] h-[calc(100vh-80px)] border-t-1 border-[#000] backdrop-blur-3xl bg-[#00000081] flex flex-col items-end'>
        ...
        <button disabled={redirected} style={addedToCartProducts.length === 0 ? {display: 'none'} : {}} className="mt-20 p-2 bg-[#105200da] rounded-[5px] cursor-pointer" onClick={phoneNumber === undefined ? handleSignInDisplay : handleOrderPageDisplay}>
